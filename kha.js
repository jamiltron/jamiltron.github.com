...
var Animation = function(frameDuration,frames,playState) {
...
Animation.prototype = {
	...
	,playState: null
	,get_isDone: function() {
		return Type.enumEq(this.playState,PlayState.Once) && this.index >= this.frames.length;
	}
	,reset: function() {
		this.index = 0;
		this.currentTime = 0;
	...
	,update: function(deltaTime) {
		...
		if(this.index >= this.frames.length) {
			if(Type.enumEq(this.playState,PlayState.Loop)) this.index = 0; else this.index = this.frames.length;
		}
	}
...
CollisionHandler.handleGroupCollisions = function(leftGroup,rightGroup,callback) {
	var _g = 0;
...
CollisionHandler.handleSingleToGroupCollisions = function(single,group,callback) {
	...
	while(_g < group.length) {
		var g = group[_g];
		++_g;
		CollisionHandler.handleBiCollision(single,g,callback);
	}
};
...
Controls.prototype = {
	...
	,keyDown: function(key,value) {
		...
		case 10:
			...
			break;
		case 6:
			if(value == "z") this.shoot = true;
			break;
		default:
		}
	}
	,keyUp: function(key,value) {
		switch(key[1]) {
		case 11:
			this.left = false;
			break;
...
var EnemyState = $hxClasses["EnemyState"] = { __ename__ : true, __constructs__ : ["Active","InActive","Exploding"] };
EnemyState.Active = ["Active",0];
EnemyState.Active.toString = $estr;
EnemyState.Active.__enum__ = EnemyState;
EnemyState.InActive = ["InActive",1];
EnemyState.InActive.toString = $estr;
EnemyState.InActive.__enum__ = EnemyState;
EnemyState.Exploding = ["Exploding",2];
EnemyState.Exploding.toString = $estr;
EnemyState.Exploding.__enum__ = EnemyState;
var Enemy = function(x,y,activeAnimation,explodeAnimation,explosionSound) {
	this.speed = 200.0;
	this.activeAnimation = activeAnimation;
	this.explodeAnimation = explodeAnimation;
	this.explosionSound = explosionSound;
	this.hitbox = new Hitbox(x,y,2,0,activeAnimation.getFrame(0).get_width() - 4,Std["int"](activeAnimation.getFrame(0).get_height() / 2));
	this.activate(x,y);
};
...
Enemy.prototype = {
	...
	,speed: null
	,get_isActive: function() {
		return !Type.enumEq(this.enemyState,EnemyState.InActive);
	}
	,set_isActive: function(value) {
		if(value) this.enemyState = EnemyState.Active; else this.enemyState = EnemyState.InActive;
		return value;
	}
	,get_width: function() {
		return this.activeAnimation.getFrame(0).get_width();
	}
	,get_height: function() {
		return this.activeAnimation.getFrame(0).get_height();
	}
	,activate: function(x,y) {
		this.x = x;
		this.y = y;
	...
	,render: function(g) {
		if(!this.get_isActive()) return;
		if(!this.animation.get_isDone()) g.drawImage(this.animation.getCurrentFrame(),this.x,this.y);
	}
	,update: function(deltaTime) {
		if(!this.get_isActive()) return;
		if(Type.enumEq(this.enemyState,EnemyState.Active)) {
			this.y += this.speed * deltaTime;
			this.hitbox.updatePosition(this.x,this.y);
		}
		this.animation.update(deltaTime);
		if(Type.enumEq(this.enemyState,EnemyState.Exploding) && this.animation.get_isDone()) this.set_isActive(false);
	}
...
var EnemySpawner = function(spawnMinTime,spawnMaxTime,minSpawnX,maxSpawnX,maxPositionY) {
	if(maxPositionY == null) maxPositionY = 600;
	if(maxSpawnX == null) maxSpawnX = 0;
	if(minSpawnX == null) minSpawnX = 0;
	this.setAssets();
...
EnemySpawner.prototype = {
	...
	,currentTime: null
	,enemies: null
	,generateRandomSpawnTime: function(spawnMinTime,spawnMaxTime) {
	...
	,reset: function() {
		...
		while(_g < _g1.length) {
			var enemy = _g1[_g];
			++_g;
			enemy.set_isActive(false);
		}
	}
	,setAssets: function() {
		this.enemyImage = kha_Assets.images.enemyShip;
		var explodeFrames = [];
		explodeFrames.push(kha_Assets.images.smokeOrange0);
		explodeFrames.push(kha_Assets.images.smokeOrange1);
		explodeFrames.push(kha_Assets.images.smokeOrange2);
		explodeFrames.push(kha_Assets.images.smokeOrange3);
		this.activeAnimation = new Animation(1,[this.enemyImage],PlayState.Loop);
		this.explodeAnimation = new Animation(0.1,explodeFrames,PlayState.Once);
		this.explodeSound = kha_Assets.sounds.enemyExplosion;
	}
	,spawn: function(x,y) {
		var _g1 = 0;
		...
		while(_g1 < _g) {
			var i = _g1++;
			if(!this.enemies[i].get_isActive()) {
	...
	,update: function(deltaTime) {
		...
		while(_g1 < _g) {
			...
			if(enemy.get_isActive() && enemy.y > this.maxPositionY) enemy.set_isActive(false);
		}
	}
	,render: function(g) {
		var _g1 = 0;
...
Gun.prototype = {
	...
	,shoot: function(x,y) {
		if(this.cooldownLeft <= 0) {
			kha_audio2_Audio1.play(this.bulletSound,false,true);
			this.cooldownLeft = this.shotInterval;
			var adjX = x - this.bulletImage.get_width() / 2;
			var _g1 = 0;
...
var GameState = $hxClasses["GameState"] = { __ename__ : true, __constructs__ : ["Initializing","MainMenu","Playing","Over"] };
GameState.Initializing = ["Initializing",0];
GameState.Initializing.toString = $estr;
GameState.Initializing.__enum__ = GameState;
GameState.MainMenu = ["MainMenu",1];
GameState.MainMenu.toString = $estr;
GameState.MainMenu.__enum__ = GameState;
GameState.Playing = ["Playing",2];
GameState.Playing.toString = $estr;
GameState.Playing.__enum__ = GameState;
GameState.Over = ["Over",3];
GameState.Over.toString = $estr;
GameState.Over.__enum__ = GameState;
...
KhaShmup.prototype = {
	...
	,loadingFinished: function() {
		...
		this.enemySpawner = new EnemySpawner(1.0,3.0,0,800,600);
		this.timer = new Timer();
		this.uiManager = new UIManager(kha_Assets.fonts.kenpixel_mini_square,20,80,30,new Rectangle(0,0,800,600));
		kha_input_Keyboard.get().notify($bind(this,this.keyDown),$bind(this,this.keyUp));
	}
	,reset: function() {
		this.controls.reset();
		this.ship.reset(400 - Std["int"](this.ship.get_width() / 2),300 - Std["int"](this.ship.get_height() / 2));
		this.enemySpawner.reset();
		this.uiManager.reset();
		this.timer.reset();
	}
	,renderMainMenu: function(g) {
		this.uiManager.renderMainMenu(g);
	}
	,renderPlaying: function(g) {
		this.enemySpawner.render(g);
		this.ship.render(g);
		this.uiManager.renderScore(g);
	}
	,renderGameOver: function(g) {
		this.uiManager.renderGameOver(g);
	}
	,render: function(framebuffer) {
		if(Type.enumEq(this.gameState,GameState.Initializing)) return;
		var g = this.backbuffer.get_g2();
		g.begin(null,KhaShmup.bgColor);
		var _g = this.gameState;
		switch(_g[1]) {
		case 1:
	...
	,handleCollisions: function() {
		var bullets = this.ship.gun.getActiveBullets();
		var enemies = this.enemySpawner.getActiveEnemies();
		CollisionHandler.handleGroupCollisions(bullets,enemies,($_=this.uiManager,$bind($_,$_.scoreUp)));
		CollisionHandler.handleSingleToGroupCollisions(this.ship,enemies,$bind(this,this.setGameOver));
	}
	...
	,setupShip: function() {
		...
		this.ship = new Ship(400 - Std["int"](shipImg.get_width() / 2),300 - Std["int"](shipImg.get_height() / 2),shipImg,kha_Assets.sounds.playerExplosion);
		this.ship.gun = new Gun(0.25,kha_Assets.images.bullet,kha_Assets.sounds.bulletShoot);
	}
	...
	,updateShip: function() {
		this.ship.update(this.controls,this.timer.deltaTime);
		if(this.ship.x < 0) this.ship.set_x(0); else if(this.ship.x + this.ship.get_width() > 800) this.ship.set_x(800 - this.ship.get_width());
		if(this.ship.y < 0) this.ship.set_y(0); else if(this.ship.y + this.ship.get_height() > 600) this.ship.set_y(600 - this.ship.get_height());
	}
	,keyDown: function(key,value) {
		this.controls.keyDown(key,value);
	}
	,keyUp: function(key,value) {
		this.controls.keyUp(key,value);
	}
	,__class__: KhaShmup
};
var Lambda = function() { };
...
Main.__name__ = true;
Main.main = function() {
	kha_System.init("KhaShmup",800,600,Main.init);
};
Main.init = function() {
...
Math.__name__ = true;
var PlayState = $hxClasses["PlayState"] = { __ename__ : true, __constructs__ : ["Loop","Once"] };
PlayState.Loop = ["Loop",0];
PlayState.Loop.toString = $estr;
PlayState.Loop.__enum__ = PlayState;
PlayState.Once = ["Once",1];
PlayState.Once.toString = $estr;
PlayState.Once.__enum__ = PlayState;
...
Ship.prototype = {
	...
	,update: function(controls,deltaTime) {
		...
		if(controls.left && !controls.right) {
			var _g = this;
			_g.set_x(_g.x - (this.speed * deltaTime | 0));
		} else if(controls.right && !controls.left) {
			var _g1 = this;
			_g1.set_x(_g1.x + (this.speed * deltaTime | 0));
		}
		if(controls.up && !controls.down) {
			var _g2 = this;
			_g2.set_y(_g2.y - (this.speed * deltaTime | 0));
		} else if(controls.down && !controls.up) {
			var _g3 = this;
			_g3.set_y(_g3.y + (this.speed * deltaTime | 0));
		}
		if(this.gun != null) {
			if(controls.shoot) this.gun.shoot(this.x + Std["int"](this.image.get_width() / 2),this.y - this.gunOffsetY);
...
var UIManager = function(font,scoreSize,titleSize,instructionSize,viewport) {
	this.font = font;
...
$hxClasses["UIManager"] = UIManager;
UIManager.__name__ = true;
UIManager.prototype = {
	font: null
	,scoreSize: null
	,titleSize: null
	...
	,renderGameOver: function(g) {
		...
		g.set_fontSize(this.titleSize);
		var x = this.viewport.x + Math.round(this.viewport.width / 2);
		var gameOverW = g.get_font().width(this.titleSize,"Game Over");
		g.drawString("Game Over",x - Math.round(gameOverW / 2),this.viewport.y + 60);
		g.set_fontSize(this.instructionSize);
		var restartW = g.get_font().width(this.instructionSize,"Press Z to restart.");
		g.drawString("Press Z to restart.",x - Math.round(restartW / 2),this.viewport.height - 100);
	}
	,renderScore: function(g) {
		g.set_font(this.font);
		g.set_fontSize(this.scoreSize);
		g.drawString("score: " + this.score,this.viewport.x + 10,this.viewport.y + 10);
	}
	,renderMainMenu: function(g) {
		g.set_font(this.font);
		g.set_fontSize(this.titleSize);
		var x = this.viewport.x + Math.round(this.viewport.width / 2);
		var titleW = g.get_font().width(this.titleSize,"Kha Shmup");
		g.drawString("Kha Shmup",x - Math.round(titleW / 2),this.viewport.y + 60);
		g.set_fontSize(this.instructionSize);
		var keysW = g.get_font().width(this.instructionSize,"Arrow Keys to move, Z key to shoot.");
		g.drawString("Arrow Keys to move, Z key to shoot.",x - Math.round(keysW / 2),this.viewport.height - 130);
		var startW = g.get_font().width(this.instructionSize,"Press Z to start.");
		g.drawString("Press Z to start.",x - Math.round(startW / 2),this.viewport.height - 100);
	}
	,__class__: UIManager
};
...
var kha_ImageList = function() {
	this.smokeOrange3Description = { files : ["http://jamiltron.com/khaShmup/smokeOrange3.png"], original_height : 93, type : "image", original_width : 93, name : "smokeOrange3"};
	this.smokeOrange3Name = "smokeOrange3";
	this.smokeOrange3 = null;
	this.smokeOrange2Description = { files : ["http://jamiltron.com/khaShmup/smokeOrange2.png"], original_height : 93, type : "image", original_width : 93, name : "smokeOrange2"};
	this.smokeOrange2Name = "smokeOrange2";
	this.smokeOrange2 = null;
	this.smokeOrange1Description = { files : ["http://jamiltron.com/khaShmup/smokeOrange1.png"], original_height : 93, type : "image", original_width : 93, name : "smokeOrange1"};
	this.smokeOrange1Name = "smokeOrange1";
	this.smokeOrange1 = null;
	this.smokeOrange0Description = { files : ["http://jamiltron.com/khaShmup/smokeOrange0.png"], original_height : 93, type : "image", original_width : 93, name : "smokeOrange0"};
	this.smokeOrange0Name = "smokeOrange0";
	this.smokeOrange0 = null;
	this.playerShipDescription = { files : ["http://jamiltron.com/khaShmup/playerShip.png"], original_height : 75, type : "image", original_width : 99, name : "playerShip"};
	this.playerShipName = "playerShip";
	this.playerShip = null;
	this.enemyShipDescription = { files : ["http://jamiltron.com/khaShmup/enemyShip.png"], original_height : 84, type : "image", original_width : 93, name : "enemyShip"};
	this.enemyShipName = "enemyShip";
	this.enemyShip = null;
	this.bulletDescription = { files : ["http://jamiltron.com/khaShmup/bullet.png"], original_height : 37, type : "image", original_width : 13, name : "bullet"};
...
kha_ImageList.prototype = {
	...
	,smokeOrange0: null
	,smokeOrange0Name: null
	,smokeOrange0Description: null
	,smokeOrange0Load: function(done) {
		kha_Assets.loadImage("smokeOrange0",function(image) {
			done();
		});
	}
	,smokeOrange0Unload: function() {
		this.smokeOrange0.unload();
		this.smokeOrange0 = null;
	}
	,smokeOrange1: null
	,smokeOrange1Name: null
	,smokeOrange1Description: null
	,smokeOrange1Load: function(done) {
		kha_Assets.loadImage("smokeOrange1",function(image) {
			done();
		});
	}
	,smokeOrange1Unload: function() {
		this.smokeOrange1.unload();
		this.smokeOrange1 = null;
	}
	,smokeOrange2: null
	,smokeOrange2Name: null
	,smokeOrange2Description: null
	,smokeOrange2Load: function(done) {
		kha_Assets.loadImage("smokeOrange2",function(image) {
			done();
		});
	}
	,smokeOrange2Unload: function() {
		this.smokeOrange2.unload();
		this.smokeOrange2 = null;
	}
	,smokeOrange3: null
	,smokeOrange3Name: null
	,smokeOrange3Description: null
	,smokeOrange3Load: function(done) {
		kha_Assets.loadImage("smokeOrange3",function(image) {
			done();
		});
	}
	,smokeOrange3Unload: function() {
		this.smokeOrange3.unload();
		this.smokeOrange3 = null;
	}
	,__class__: kha_ImageList
};
var kha_SoundList = function() {
	this.playerExplosionDescription = { files : ["http://jamiltron.com/khaShmup/playerExplosion.ogg"], type : "sound", name : "playerExplosion"};
	this.playerExplosionName = "playerExplosion";
	this.playerExplosion = null;
	this.enemyExplosionDescription = { files : ["http://jamiltron.com/khaShmup/enemyExplosion.ogg"], type : "sound", name : "enemyExplosion"};
	this.enemyExplosionName = "enemyExplosion";
	this.enemyExplosion = null;
	this.bulletShootDescription = { files : ["http://jamiltron.com/khaShmup/bulletShoot.ogg"], type : "sound", name : "bulletShoot"};
	this.bulletShootName = "bulletShoot";
...
kha_SoundList.prototype = {
	bulletShoot: null
	,bulletShootName: null
	,bulletShootDescription: null
	,bulletShootLoad: function(done) {
		kha_Assets.loadSound("bulletShoot",function(sound) {
			done();
		});
	}
	,bulletShootUnload: function() {
		this.bulletShoot.unload();
		this.bulletShoot = null;
	}
	,enemyExplosion: null
	,enemyExplosionName: null
	,enemyExplosionDescription: null
	,enemyExplosionLoad: function(done) {
		kha_Assets.loadSound("enemyExplosion",function(sound) {
			done();
		});
	}
	,enemyExplosionUnload: function() {
		this.enemyExplosion.unload();
		this.enemyExplosion = null;
	}
	,playerExplosion: null
	,playerExplosionName: null
	,playerExplosionDescription: null
	,playerExplosionLoad: function(done) {
		kha_Assets.loadSound("playerExplosion",function(sound) {
			done();
		});
	}
	,playerExplosionUnload: function() {
		this.playerExplosion.unload();
		this.playerExplosion = null;
	}
...
var kha_FontList = function() {
	this.kenpixel_mini_squareDescription = { files : ["http://jamiltron.com/khaShmup/kenpixel_mini_square.ttf"], type : "font", name : "kenpixel_mini_square"};
...
KhaShmup.gunSpeed = 0.25;
UIManager.scoreX = 10;
UIManager.scoreY = 10;
UIManager.titleY = 60;
UIManager.titleStr = "Kha Shmup";
UIManager.keysStr = "Arrow Keys to move, Z key to shoot.";
UIManager.keysY = 130;
UIManager.startStr = "Press Z to start.";
UIManager.startY = 100;
UIManager.gameOverStr = "Game Over";
UIManager.restartStr = "Press Z to restart.";
